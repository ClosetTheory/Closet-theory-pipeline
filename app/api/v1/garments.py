...
# --- TEMPORARY: one-off dev -> prod data sync -------------------------------------------------
# The canonical-image repair (605 regenerated images), the embedding backfill (1176 vectors that
# were hash-seeded noise) and the attribute cleanups were all carried out against the dev
# database. Production runs the same code against the same garment ids but still holds the old
# data, so its duplicate detection and similarity retrieval are still comparing random numbers.
#
# Regenerating on production would mean paying for ~605 image generations a second time, so this
# copies the finished result across instead. Scoped to the caller's own tenant, additive (it
# never deletes), and to be removed once the sync is confirmed - same playbook as the earlier
# one-off import endpoints.

class SyncGarmentPayload(BaseModel):
    garment_id: str
    canonical_image_b64: Optional[str] = Field(default=None, description="New canonical image bytes, base64")
    embedding: Optional[List[float]] = Field(default=None, description="Replacement embedding vector")
    attributes: Optional[Dict[str, Any]] = Field(default=None, description="Replacement attributes_json")


@router.post("/_sync_from_dev")
async def sync_from_dev(
    payloads: List[SyncGarmentPayload],
    current_user: User = Depends(get_current_user),
    session: AsyncSession = Depends(get_db_session),
    storage: StorageClient = Depends(get_storage),
):
    """TEMPORARY: apply repaired canonical images / embeddings / attributes from dev."""
    ...
    for item in payloads:
        garment = await session.get(Garment, item.garment_id)
        if not garment or garment.tenant_id != current_user.tenant_id:
            skipped.append({"garment_id": item.garment_id, "reason": "not found or other tenant"})
            continue

        changed = []

        if item.canonical_image_b64:
            raw = _b64.b64decode(item.canonical_image_b64)
            key = f"canonical/{garment.tenant_id}/{garment.id}_canonical_synced.jpg"
            uri = await storage.put_object(key, raw, content_type="image/jpeg")
            with _PILImage.open(_io.BytesIO(raw)) as img:
                width, height = img.size
            asset = ImageAsset(
                tenant_id=garment.tenant_id,
                member_id=garment.member_id,
                object_uri=uri,
            ...
            session.add(asset)
            await session.flush()
            garment.canonical_image_id = asset.id
            changed.append("canonical_image")
        ...
        if item.embedding:
            ...
            record.dimension = len(item.embedding)
            changed.append("embedding")

        if item.attributes is not None:
            garment.attributes_json = item.attributes
            changed.append("attributes")

        applied.append({"garment_id": garment.id, "changed": changed})

    await session.commit()
    return {"applied": len(applied), "skipped": len(skipped), "details": applied[:5], "skipped_details": skipped[:5]}
