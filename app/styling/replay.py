"""Replays a persisted StylingRequest's outfits back into a StylingRecommendationResponse.

Extracted from app/api/v1/styling.py::get_styling_request so the same "read back what was
already generated" logic can be shared with Outfit-of-the-Day's cache-hit path (app/styling/
ootd.py) without duplicating it.
"""
